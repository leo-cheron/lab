...
import MainShaderMaterial from "scene/materials/MainShaderMaterial";
import MainVsDepth from "scene/shaders/MainVs_depth.glsl";
import MainVs from "scene/shaders/MainVs_depth.glsl";
	...
	load(src, callback, type)
	{
		if(this._geometries[src]) 
			...
			this._loader.load(src, (geometry) =>
			{
				...
				var mesh = new THREE.Mesh(this.split(geometry), this.material);

				mesh.wireframe = new THREE.LineSegments(new THREE.WireframeGeometry(geometry), new THREE.LineBasicMaterial( { color: this._model.wireframeColor } ));
	...
	build(type, callback)
	{
		switch(type)
		{
			...
			case MeshFactory.TYPE_BED:
				this.load("mesh/set-bed.js", callback, type);
				break;

			case MeshFactory.TYPE_BRICKELL:
				this.load("mesh/set-brickell.js", callback, type);
				break;
			
			case MeshFactory.TYPE_CREST:
				this.load("mesh/set-crest.js", callback, type);
				break;

			case MeshFactory.TYPE_CUPS:
				this.load("mesh/set-cups.js", callback, type);
				break;
			
			case MeshFactory.TYPE_EAGLE:
				this.load("mesh/set-eagle.js", callback, type);
				break;

			case MeshFactory.TYPE_EASEL:
				this.load("mesh/set-easel.js", callback, type);
				break;

			case MeshFactory.TYPE_HELMET:
				this.load("mesh/helmet.js", callback, type);
				break;

			case MeshFactory.TYPE_HORN:
				this.load("mesh/set-horn.js", callback, type);
				break;

			case MeshFactory.TYPE_MAP:
				this.load("mesh/set-map.js", callback, type);
				break;

			case MeshFactory.TYPE_MUSE:
				this.load("mesh/set-muse.js", callback, type);
				break;

			case MeshFactory.TYPE_SCROLL:
				this.load("mesh/set-scroll.js", callback, type);
				break;

			case MeshFactory.TYPE_SWORDS:
				this.load("mesh/set-swords.js", callback, type);
				break;
		}
	}
}
